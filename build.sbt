...
lazy val `scalameta-ast` = projectMatrix
  ...
  .jsPlatform(
    ...
    settings = Def.settings(
      jsProjectSettings,
      commonLatest,
      libraryDependencies += "org.ekrich" %%% "sconfig" % "1.12.0",
      libraryDependencies += ("com.github.xuwei-k" %%% "scalafmt-core" % "3.9.10-fork-1").withSources(),
    )
  )
...
lazy val localServer = project.settings(
  ...
  libraryDependencies ++= Seq(
    ...
    "org.scalatest" %%% "scalatest-freespec" % "3.2.19" % Test,
    "com.microsoft.playwright" % "playwright" % "1.55.0" % Test,
  )
)
